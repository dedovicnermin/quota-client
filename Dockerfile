...
RUN $JAVA_HOME/bin/jdeps \
  ...
  --print-module-deps \
  --class-path target/* \
  target/quota-client-jar-with-dependencies.jar > jre-deps.info
...
RUN $JAVA_HOME/bin/jlink \
    --verbose \
    --add-modules $(cat jre-deps.info),java.management,jdk.management.agent,jdk.httpserver,java.instrument \
    --strip-debug \
    --no-man-pages \
    --no-header-files \
    --compress=2 \
